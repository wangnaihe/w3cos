...
pub fn generate_skeleton(bundle: &EsmBundle) -> String {
    ...
    out.push_str(&format!("// modules: {}, symbols: {}\n\n", bundle.modules.len(), bundle.symbols.len()));
...
fn generate_module(bundle: &EsmBundle, module: &BundledModule) -> String {
    ...
    for (local, bundled) in &module.local_to_bundled {
        ...
        if let Some(owner) = bundle.modules.iter().find(|m| {
            ...
            out.push_str(&format!(
                "    pub use super::{}::{} as {};\n",
                owner.namespace, bundled, sanitize_ident(local)
            ));
        }
    }
...
enum TopLevelItem {
    ...
    Class { name: String, methods: Vec<(String, Vec<Stmt>)> },
}
...
fn find_function_body(items: &[TopLevelItem], name: &str, renames: &[(String, String)]) -> String {
    for item in items {
        if let TopLevelItem::Function { name: fn_name, body } = item {
            if fn_name == name {
...
export function keymap() {}"#,
        ...
        assert!(code.contains("mod m0"), "should emit namespaced modules: {code}");
        // Classes become structs.
        assert!(code.contains("pub struct"), "classes should become structs: {code}");
        assert!(code.contains("EditorView"), "EditorView should appear: {code}");
        assert!(code.contains("EditorState"), "EditorState should appear: {code}");
        // Functions become fns.
        assert!(code.contains("pub fn"), "functions should become fns: {code}");
        assert!(code.contains("keymap"), "keymap fn should appear: {code}");
        // Cross-module imports become use re-exports.
        assert!(code.contains("pub use super::"), "cross-module bindings should re-export: {code}");
...
export function boot() {}"#,
        ...
        std::fs::write(lib.join("index.js"), r#"export class Theme {}
export function makeTheme() {}"#).unwrap();
        ...
        let wrapper = format!("#![allow(dead_code, unused_imports, clippy::all)]\nfn main() {{}}\n{code}");
...
export function boot() {
  ...
  mount() {
    ...
    return el;
        ...
        assert!(code.contains("EditorView::new("), "new X() should lower to X::new(): {code}");
        assert!(code.contains("return view"), "return should be lowered: {code}");
        // Class method body should be lowered
        assert!(code.contains("document.createElement"), "method body should be lowered: {code}");
        // Should NOT have todo!() for functions that have bodies
        assert!(!code.contains("todo!(\"lower ESM body: boot\")"), "boot body should be lowered: {code}");

...
fn test_ios_web_emit_has_actions_and_binds() {
    ...
    assert!(html.contains("data-action=\"increment:taps\""), "tap action");
...
<Column className="box" style={{ gap: 8 }}>
  <Text>Hi</Text>
    ...
    assert!(html.contains("padding:20px") || html.contains("padding: 20px"), "merged padding in inline: {html}");
    assert!(html.contains("gap:16px") || html.contains("gap: 16px") || html.contains("gap:8px"), "gap in inline: {html}");
    ...
    assert!(main_rs.contains("padding: Edges::all(20_f32)"), "native merged padding: {main_rs}");
    assert!(main_rs.contains("gap: 16_f32") || main_rs.contains("gap: 8_f32"), "native gap: {main_rs}");
}

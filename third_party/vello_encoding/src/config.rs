...
impl BufferSizes {
    pub fn new(layout: &Layout, workgroups: &WorkgroupCounts) -> Self {
        ...
        // Keep Vello's proven capacities on mobile too. Undersizing any of
        // these scratch buffers can silently produce a fully black frame on
        // Metal even though command submission succeeds.
        let (bin_data, tiles, lines, seg_counts, segments, blend_spill, ptcl) = (
            ...
            BufferSize::new(1 << 21),
            BufferSize::new(1 << 21),
            // 16 * 16 (1 << 8) is one blend spill, allowing 4096 spills.
            BufferSize::new(1 << 20),
            BufferSize::new(1 << 23),
        );
        Self {
